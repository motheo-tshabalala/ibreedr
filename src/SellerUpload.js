...
export default function SellerUpload() {
  ...
  const isStepValid = () => {
    switch (currentStep) {
      ...
      case 2: return formData.name && formData.animal_type && formData.breed_type;
      case 3: return true;
      case 4: return true;
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                className={`p-6 rounded-xl border-2 transition-all ${listingType === 'individual'
              ...
              >
                ...
                <div className="text-sm text-muted-foreground mt-0.5">Single livestock</div>
              </button>

              <button
                type="button"
                onClick={() => setListingType('bundle')}
                className={`p-6 rounded-xl border-2 transition-all ${listingType === 'bundle'
                    ? 'border-primary bg-primary/5'
                    : 'border-border hover:border-primary/50'
                  }`}
              >
                ...
                <div className="text-sm text-muted-foreground mt-0.5">Multiple animals, one price</div>
              </button>
            </div>
          </div>
        );
              ...
              <div className="mt-2 p-3 bg-primary/5 rounded-lg border border-primary/20">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-1">
                    ...
                    <p className="text-xs font-medium text-primary">Market Price Suggestion</p>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    as of {new Date(marketSuggestion.effective_date).toLocaleDateString()}
                  </p>
                </div>
                <p className="text-sm font-semibold">
                  R {marketSuggestion.price_min} - R {marketSuggestion.price_max} / {marketSuggestion.price_unit}
                </p>
                {marketSuggestion.grade && (
                  <p className="text-xs text-muted-foreground mt-1">Grade: {marketSuggestion.grade}</p>
                )}
                <p className="text-xs text-muted-foreground mt-1">
                  Source: {marketSuggestion.source}
                </p>
                <button
                  type="button"
                  onClick={() => {
                    const suggestedPrice = Math.round((marketSuggestion.price_min + marketSuggestion.price_max) / 2);
                    setFormData({ ...formData, price: suggestedPrice.toString() });
                  }}
                  className="mt-2 text-xs text-primary hover:underline"
                >
                  Use suggested price (R {Math.round((marketSuggestion.price_min + marketSuggestion.price_max) / 2)})
                </button>
              </div>
            )}
          </div>
        ...
        return (
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input type="tel" value={formData.seller_phone} onChange={(e) => setFormData({ ...formData, seller_phone: e.target.value })} placeholder="+27 XX XXX XXXX" className="mt-1" />
            </div>
            <div>
              <Label>Facebook (Optional)</Label>
              <Input type="url" value={formData.facebook_url} onChange={(e) => setFormData({ ...formData, facebook_url: e.target.value })} placeholder="https://facebook.com/yourfarm" className="mt-1" />
            </div>
            <div>
              <Label>Instagram (Optional)</Label>
              <Input type="url" value={formData.instagram_url} onChange={(e) => setFormData({ ...formData, instagram_url: e.target.value })} placeholder="https://instagram.com/yourfarm" className="mt-1" />
            </div>
            <div>
              <Label>WhatsApp (Optional)</Label>
              <Input type="tel" value={formData.whatsapp_number} onChange={(e) => setFormData({ ...formData, whatsapp_number: e.target.value })} placeholder="+27 XX XXX XXXX" className="mt-1" />
            </div>
            <div>
              <Label>Website (Optional)</Label>
              <Input type="url" value={formData.website_url} onChange={(e) => setFormData({ ...formData, website_url: e.target.value })} placeholder="https://yourfarm.com" className="mt-1" />
            </div>
          </div>
        );
        ...
        return (
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2 mb-3">
                {formData.images.map((url, index) => (
                  <div key={index} className="relative aspect-square rounded-lg overflow-hidden bg-muted">
                    ...
                    <button type="button" onClick={() => removeImage(index)} className="absolute top-1 right-1 p-1 bg-background/80 rounded-full">
                      <X className="w-3 h-3" />
                    </button>
                  </div>
                ))}
            ...
            <div className="border-2 border-dashed border-border rounded-lg p-4 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-2">Upload a video (Optional)</p>
    ...
    <div className="min-h-screen bg-background">
      <div className="bg-card border-b sticky top-0 z-30">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold">List Your Livestock</h1>
        </div>
      </div>
          ...
          <CardContent className="p-6">
            ...
            <div className="flex justify-between gap-4 mt-8">
              ...
              {currentStep < totalSteps ? (
                ...
                <Button onClick={handleSubmit} disabled={submitting || !isStepValid()} className="gap-2 ml-auto">
